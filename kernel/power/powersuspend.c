...
static int mode;  // Current powersave more  (kernel / userspace)
...
void set_power_suspend_state_hook(int new_state)
{
  if (mode == POWER_SUSPEND_KERNEL)
    set_power_suspend_state(new_state);  //Only allow kernel hook changes in kernel mode
}
    ...
    struct kobj_attribute *attr, const char *buf, size_t count)
{
  ...
  if (mode != POWER_SUSPEND_USERSPACE) //Only allow sysfs changes in userspace mode
    return -EINVAL;
